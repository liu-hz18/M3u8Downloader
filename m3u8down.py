...
class M3u8Download:
    ...
    def __init__(self, url, name, max_workers=12, num_retries=5):
        self.url = url
        self.name = name
    ...
    def output_mp4(self):
        ...
        os.system(cmd)
        #  os.system(f"ffmpeg -i large_{self.name}.mp4 -r 10 -b:a 32k {self.name}.mp4")
        #  os.remove(f"./large_{self.name}.mp4")
        os.remove(f"./{self.name}.m3u8")
        shutil.rmtree(f"./{self.name}")
        print(f"Download successfully --> {self.name}")


if __name__ == "__main__":
    # "https://www.bilibili.com/example/index.m3u8"
